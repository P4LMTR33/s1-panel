...
/// D-Bus proxy for the Firebat Panel Daemon.
#[proxy(
    interface = "org.firebat.Panel1",
    default_service = "org.firebat.Panel",
    default_path = "/org/firebat/Panel"
)]
trait Daemon1 {
...
impl DaemonClient {
                ...
                if let Ok(conn) = Connection::session().await {
                    ...
                    if Self::service_exists(&conn).await {
                        ...
                        if Self::service_exists(&sys_conn).await {
                            ...
                            anyhow::bail!("Daemon service not found on session or system bus. Is firebat-paneld running?")
                        }
                    }
                } else {
    ...
    async fn service_exists(conn: &Connection) -> bool {
        ...
        if let Ok(dbus_proxy) = DBusProxy::new(conn).await {
            dbus_proxy
                .name_has_owner("org.firebat.Panel".try_into().unwrap())
                .await
                .unwrap_or(false)
        } else {
            false
