...
//!
//! Provides the `org.firebat.Panel1` interface.

use std::sync::Arc;

use firebat_panel_hw::{lcd::parse_hex_color, Orientation};
...
/// D-Bus interface implementation for the Firebat Panel Daemon.
...
#[interface(name = "org.firebat.Panel1")]
impl Daemon1Interface {
...
pub async fn run_dbus_server(
    ...
    connection
        .object_server()
        .at("/org/firebat/Panel", interface)
        .await
        .map_err(|e| anyhow::anyhow!("Failed to register object: {}", e))?;

    connection
        .request_name("org.firebat.Panel")
        .await
        .map_err(|e| anyhow::anyhow!("Failed to request bus name: {}", e))?;

    info!(
        "D-Bus service registered at org.firebat.Panel on {} bus",
        bus_name
    );
    Ok(connection)
}
