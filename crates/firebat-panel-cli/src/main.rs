//! Firebat Panel Control Tool
//!
//! CLI for controlling the Firebat Panel daemon via D-Bus.

use anyhow::{Context, Result};
use clap::{Parser, Subcommand};
use firebat_panel_client::{BusType, DaemonClient};
...
#[derive(Parser)]
#[command(name = "firebat-panelctl")]
#[command(about = "Control tool for Firebat Panel daemon")]
#[command(version)]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    let client = DaemonClient::connect_with_bus(cli.bus.into())
        .await
        .context("Failed to connect to daemon. Is firebat-paneld running?")?;
